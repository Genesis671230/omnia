...
// function ProofRow({ t, order, missing, open, onToggle, zoho, originalCurrency }: {
//   t: ReconTxn;
...
// }) {
//   const colCount = originalCurrency ? 6 : 5;
//   return (
//     <>
//       <tr
//         onClick={onToggle}
...
//         </td>
//         {originalCurrency && (
//           <td className="px-3 py-2 text-right font-mono tabular-nums text-[#8A8175]">
...
//     let alive = true;
//     fetch(`/api/reconcile/line/${encodeURIComponent(r.id)}/invoices`)
//       .then((x) => x.json())
//       .then((d: { statuses?: Record<string, InvoiceStatus> }) => {
//         if (!alive || !d.statuses) return;
//         setZohoByRef((prev) => {
//           const next = new Map(prev);
//           for (const [ref, iv] of Object.entries(d.statuses!)) {
...
function ProofRow({
  ...
  canSelect, selected, onSelectToggle, onRecordOne, posting,
}: {
  t: ReconTxn;
  order: OrderDetail | undefined;
  missing: boolean;
