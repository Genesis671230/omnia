...
import { ZohoSyncRunsRepository, type ZohoSyncMode } from "@/lib/repositories/zoho-sync-runs.repository";
...
async function runSyncCycle(trigger: "scheduler" | "manual") {
  // Most cycles page only what changed. A full pass costs 278 Zoho requests
  // against a ~5,000/day org cap, so it is reserved for the nightly run that
  // catches deletions — see nextSyncWindow().
  let mode: ZohoSyncMode = "full";
  try {
    const window = await ZohoSyncRunsRepository.nextSyncWindow();
    mode = window.mode;
    console.log(`[zoho-sync] ${window.mode} cycle — ${window.reason}`);

    const { results, zohoWatermark } = await syncZohoAndInventory({ sinceIso: window.sinceIso });
    await ZohoSyncRunsRepository.record({ trigger, sourceResults: results, mode, watermark: zohoWatermark });
  } catch (e) {
    await ZohoSyncRunsRepository.record({ trigger, sourceResults: [], error: (e as Error).message, mode }).catch(() => {
...
export function startZohoSyncScheduler() {
  ...
  const minutes = Math.max(parseInt(process.env.ZOHO_SYNC_INTERVAL_MINUTES || "", 10) || DEFAULT_INTERVAL_MINUTES, 30);
